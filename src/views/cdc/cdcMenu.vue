      ...
      <Menu :active-name="projectNameSelected" @on-select="projectSelected" theme="dark" ref="menu">
        <MenuItem v-for="item in projectList" :key="item.name" :name="item.name">
          <p class="Project-menu-item">
            <span>{{ item.name }}</span>
            <!-- <Icon type="ios-trash" @click="projectDeleteForm(item)" /> -->
          </p>
        </MenuItem>
      </Menu>
    </div>
...
export default {
  ...
  computed: {
    ...mapState(["projectNameSelected", "projectServerData"]),
    projectList() {
      return Object.keys(this.projectServerData).map(item => ({ name: item }));
    }
  },
  watch: {},
  methods: {
    ...mapMutations(["projectNameSelectedChange", "projectServerDataChange"]),
    ...mapActions(["projectServerDataRefresh"]),
    projectSelected(name) {
      if (name === this.selectMenu) return;
